				...
				<table class="table table-striped ">
					...
					<tbody>
						...
						<tr>
							...
							<td>{{ $value->status == '1' ? 'Utama' : 'Lain-nya' }}</td>
							<td class="text-center">
								<a href="#" class="btn bg-primary rounded-round" title="Edit" onclick="getEditMenu({{ $value->id }})"><i class="icon-pen-plus"></i></a>
								<a href="#" class="btn bg-danger rounded-round" href="{{ route('product-image.deletepicture', $value->id) }}" onclick="event.preventDefault(); document.getElementById('destroy-form-{{ $value->id }}').submit();"> <i class=" icon-trash"></i></a>
		                        <form id="destroy-form-{{ $value->id }}" action="{{ route('product-image.deletepicture', $value->id) }}" method="POST" style="display: none;">
		                        		{{ method_field('DELETE') }}
		                                @csrf
		                        </form>
							</td>
...
<div id="modal_theme_info" class="modal fade"  tabindex="-1" style="display: none;" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<form action="{{ route('product-image.createpicture') }}" method="POST" id="formData" class="form-validate-jquery" enctype="multipart/form-data">
				<div class="modal-body">
				   	@csrf

				   	<input type="hidden" name="id_product" value="{{ request()->segment(3) }}">
						...
						<div class="col-lg-12">
							<div class="media no-margin-top">
								...
								<div class="media-body">
									...
									<span class="help-block">Accepted formats: jpg. Max file size 2Mb. Usahakan file sekecil mungkin dan jelas. </span>
								</div>
							</div>
						</div>
					</div>
...
<script>
	...
	function addUser(){
		var red = "{{ route('product-image.createpicture') }}";
	...
	function getEditMenu(id) {
		$('form').trigger('reset');
		$('#modal_theme_info').modal('show');
		var url = "{{ route('product-image.getpicture', ['id' => 'idMenuEdit']) }}".replace("idMenuEdit", id);
		var red = "{{ route('product-image.updatepicture', ['id' => 'idMenu']) }}".replace("idMenu", id);
