			...
			<div class="table-responsive">
				<table class="table table-striped ">
					...
					<tbody>
						...
						<tr>
							<td><i class="fa fa-{{ $value->icon }}"></i> &nbsp;{{ $value->icon }}</td>
							<td>{{ $value->head }}</td>
							<td>{{ $value->detail }}</td>
							<td class="text-center">
								
								<a href="#" class="btn bg-primary rounded-round" title="Edit" onclick="getEditMenu({{ $value->id }})"><i class="icon-pen-plus"></i></a>
								<a href="#" class="btn bg-danger rounded-round" href="{{ route('service-detail.destroy', $value->id) }}" onclick="event.preventDefault(); document.getElementById('destroy-form-{{ $value->id }}').submit();"> <i class=" icon-trash"></i></a>
		                        <form id="destroy-form-{{ $value->id }}" action="{{ route('service-detail.destroy', $value->id) }}" method="POST" style="display: none;">
		                        		{{ method_field('DELETE') }}
		                                @csrf
		                        </form>
							</td>
...
<div id="modal_theme_info" class="modal fade"  tabindex="-1" style="display: none;" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<form action="{{ route('service-detail.store') }}" method="POST" id="formData" class="form-validate-jquery">
				<div class="modal-body">
					...
					<div class="form-group">
						<label>Icon Menu <span>  / <a target="_blank" href="https://fontawesome.com/v4.7.0/icons/"> Referensi icon</a></span></label>
						<input type="text" name="icon" id="icon" class="form-control" required placeholder="Icon Menu">
					</div>
				</div>
...
<script>
	...
	function addUser(){
		var red = "{{ route('service-detail.store') }}";
	...
	function getEditMenu(id) {
		$('form').trigger('reset');
		$('#modal_theme_info').modal('show');
		var url = "{{ route('service-detail.show', ['id' => 'idMenuEdit']) }}".replace("idMenuEdit", id);
		var red = "{{ route('service-detail.update.fix', ['id' => 'idMenu']) }}".replace("idMenu", id);
